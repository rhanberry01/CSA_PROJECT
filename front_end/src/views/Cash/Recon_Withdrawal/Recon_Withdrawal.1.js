...
//-----START OF TABLE PART
loadDeposits() {
  axios.get('http://192.168.0.188:4001/reconwithdrawal/getdeposit')
    .then(res => {
...
axios.get('http://192.168.0.188:4001/reconwithdrawal/deposittypedropdown')
.then(res => {
...
axios.get('http://192.168.0.188:4001/reconwithdrawal/bankdropdown')
.then(res => {
...
axios.get('http://192.168.0.188:4001/reconwithdrawal/getsalescash', {
params:{ 
...
axios.get('http://192.168.0.188:4001/reconwithdrawal/getsalesdeposited', {
 params:{ 
     remittance_date: moment(date).format('YYYY-MM-DD')
     }
   })
 ...
 .catch(function (error) {
  ...
  axios.get('http://192.168.0.188:4001/reconwithdrawal/getsaleswithdrawal', {
   params:{ 
       remittance_date: moment(date).format('YYYY-MM-DD')
       }
     })
     ...
     const Deposit = {
       ...
       if (a > b) {
        Swal({
         ...
         console.log(Deposit);
         axios.post('http://192.168.0.188:4001/reconwithdrawal/adddeposit', { 
           bal: Deposit.bal,
...
render() {
  ...
  console.log("go1");
    return (
      <Switch>
       <Route path="/cash/cash_withdrawal" component={ReconWithdrawal}/>
      </Switch>
      );
    }
  else{
    ...
    <CardBody>
      ...
      <Row>
          <Col>
            ...
            {<h1>Transaction #: {this.state.selectedx.map((number) => number).join(',')}</h1>}
  ...
  class Pagis extends React.PureComponent {
    ...
    toggle(checkboxdata) {
     ...
     console.log(checkboxdata);
      ...
      this.setState({
        ...
        idx: checkboxdata,
        amount: checkboxdata._net_amount,
        memo: checkboxdata.memo_,
        salesDate: moment(checkboxdata.date_created),
        depositDate: moment(checkboxdata.transaction_date),
        aria_trans_gl_code: checkboxdata.aria_trans_gl_code,
        transaction_detail_type:checkboxdata.transaction_type,
    ...
    if(checkboxdata!=''){
          axios.get('http://192.168.0.188:4001/reconwithdrawal/getselecteddeposit', {
    ...
    handleSubmit(e) {
      ...
      console.log(a);
        ...
        Swal({
          ...
          if (result.value) {
          axios.put('http://192.168.0.188:4001/reconwithdrawal/updatedeposit', { 
          id: Deposit.id,
          bal: Deposit.bal,
          amount: Deposit.amount,
          memo: Deposit.memo,
  ...
  //==================================================
    ...
    handleDelete(Depositdata) {
      ...
      Swal({
        ...
        if (result.value) {
        //console.log(Deposit);
        axios.delete('http://192.168.0.188:4001/reconwithdrawal/deletedeposit', {
        data:{ 
            id: Deposit.id
          }
        })
      ...
      */
      ...
      }, 
      {
          ...
          formatter: (cellContent, row) => (
            <Button outline color="success" size="sm" onClick={() => this.openModal(row)}><i className="fa fa-pencil-square-o"></i>&nbsp; Process</Button>
          )
    }, 
    {
          dataField: 'View',
          text: 'View',
          formatter: (cellContent, row) => (
            <Button type="submit" outline color="primary" size="sm" onClick={() => this.handleDelete(row)}><i className="fa fa-pencil-square-o"></i>&nbsp; View</Button>
          )
    }
   ];
    ...
    if (this.state.toRedirect === true){
      ...
      <div>
       ...
       <PaymentModal/>
          ...
          <Button color="success" className="float-left" onClick={() => this.toggle(this.state.selectedx.map((selectedids) => selectedids).join(','))}><i className="fa fa-plus-square"></i>&nbsp; Reconcile Withdrawal</Button>
