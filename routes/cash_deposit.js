...
router.get('/getdeposit', function (req, res, next) {
  ...
  res.locals.mysql_connection_91.query(
    ...
    (select bank_account_name from  0_all_bank_accounts where account_code =  csh.aria_trans_gl_code) as banks
...
router.post('/updatedeposit', function (req, res, next) {
  ...
  upload(req, res, (err) => {
    ...
    if (req.file == '' || req.file == undefined) {
      ...
      var file = req.file.filename;
      //  console.log('pasok2');
      var querfile = `,attachment_name= '` + file + `'`;
    }
    var quer = `UPDATE cash_deposit2.0_central_sales_audit_header 
    ...
    deposit_date='`+ req.body.date_deposited + `', 
    memo_= '`+ req.body.memo + `' 
    `+ querfile + `
    where id = '`+ req.body.id + `'`;
    //console.log(quer);

    res.locals.mysql_connection_91.query(quer,
      function (error, results, fields) {
        if (error) throw error;
        res.send(JSON.stringify(results));
      });
